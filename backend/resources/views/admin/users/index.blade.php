            ...
            <tr class="border-t">
                ...
                <td class="p-2">
                    <div class="flex justify-center gap-2">
                        <a href="{{ route('users.edit', $user) }}" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Editar</a>

                        <form action="{{ route('users.destroy', $user) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700" onclick="return confirm('¿Seguro que quieres eliminar este usuario?')">Eliminar</button>
                        </form>
                    </div>
                </td>
            </tr>
